...
async def on_message(message):
    ...
    if message.content.startswith("/페일") or message.content.startswith("/족제비"):
        await client.send_file(message.channel, '3551705276_XmsDEv60_fal4.mp4')
        channel = message.channel
        embed = discord.Embed(
            title = '페일 | 담비',
            description = '"찎찎찎"',
        ...
        embed.add_field(name='등급', value='★★★★★★★★', inline=True)
        embed.add_field(name='종류', value='담비', inline=True)
        embed.add_field(name='제조시간', value='숲속', inline=True) 
        embed.add_field(name='스킬', value='화력전개' + '\n' + '귀여워진다', inline=True)  
        embed.add_field(name='버프(FAL 한정)', value='FAL의 목베개가되어 FAL의 능력치가 상승한다', inline=True)
        print('페일')
